...
useEffect(()=>{
 ...
 const render=()=>{
  ...
  for(const el of scenes){const r=el.getBoundingClientRect();el.style.setProperty('--view',clamp((innerHeight-r.top)/(innerHeight+r.height)).toFixed(4));}
 ...
 document.querySelectorAll('.card,.intro,.rhythm,.closing,[data-scene]').forEach(el=>observer.observe(el));
